...
int* SLIC(ImageBase &src, int k, int m, std::vector<PixelLAB>& res_clusters) {
        ...
        for (int a = -1; a <= 1; a++) {
            for (int b = -1; b <= 1; b++) {
                ...
                if (_x > 0 && _x < height - 1 && _y > 0 && _y < width - 1) {
                    double grad = distance2(imageLAB[idx(_x - 1, _y, width)] - imageLAB[idx(_x + 1, _y, width)],
                                             imageLAB[idx(_x, _y - 1, width)] - imageLAB[idx(_x, _y + 1, width)]);
                    if (min > grad) {
                        min = grad;
                        min_pos = p;
                    }
    ...
    // Post-processing to assign labels to orphaned pixels using a connected component algorithm
    bool orphan = false;
    do {
        for (int i = 0; i < N; i++) {
            if (labels[i] == -1) {
...
std::vector<ClusterCenter> mergeSuperpixels(int* labels, ClusterCenter* centers, double threshold, int N) {
    std::vector<std::vector<int>> mergedClusters; // Stores indices of merged superpixels
    std::vector<bool> merged(N, false); // Indicates if a superpixel has been merged

    for (int i = 0; i < N; ++i) {
        if (!merged[i]) {
            std::vector<int> mergedCluster; // Stores indices of superpixels merged in this iteration
            mergedCluster.push_back(i); // Add current superpixel to merged cluster
            merged[i] = true; // Mark superpixel as merged

            // Iterate through remaining superpixels to see if they are similar
            for (int j = i + 1; j < N; ++j) {
                // Check similarity between superpixels
                if (!merged[j] && distance2(centers[labels[i]], centers[labels[j]]) < threshold) {
                    mergedCluster.push_back(j); // Add superpixel to this merged cluster
                    merged[j] = true; // Mark superpixel as merged
                }
            }

            // Add merged cluster to list of merged clusters
            mergedClusters.push_back(mergedCluster);
        }
    }

    // Merge superpixels using color and position means
